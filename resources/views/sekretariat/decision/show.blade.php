...
<div class="flex justify-between items-center mb-4">
    <div>
        ...
        <p class="text-gray-500 text-sm">
            {{ $existingDecision ? 'Lihat histori & tetapkan keputusan' : 'Tetapkan keputusan akhir' }}
        </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div>
            ...
            <p>
                ...
                @elseif($isRevisionRound && $isComplete)
                    <span class="text-orange-600 font-semibold">Menunggu Keputusan Babak {{ $existingDecision->round + 1 }}</span>
                @elseif($isRevisionRound)
                    <span class="text-blue-600 font-semibold">Menunggu Revisi/Review Babak {{ $existingDecision->round }}</span>
                @else
...
<div class="bg-white rounded-xl shadow-sm p-5 mb-6">
    ...
    @forelse($protocol->documents as $doc)
        <div class="flex justify-between items-center p-3 bg-gray-50 rounded-lg mb-2">
            <span class="font-medium">{{ $doc->name ?? $doc->type ?? 'Dokumen' }}</span>
    ...
    @if($protocol->revisions->isNotEmpty())
        <h3 class="font-semibold mt-4 mb-2 text-orange-700">Dokumen Revisi dari Peneliti</h3>
        @foreach($protocol->revisions as $rev)
            <div class="flex justify-between items-center p-3 bg-orange-50 rounded-lg mb-2">
                <div>
                    <span class="font-medium">{{ $rev->original_filename ?: 'Dokumen Revisi' }}</span>
                    <p class="text-xs text-gray-500">{{ $rev->submitted_at?->translatedFormat('d M Y, H:i') }}</p>
                </div>
...
<!-- Histori Keputusan Sekretariat (semua babak) -->
@if($decisionHistory->isNotEmpty())
    <div class="bg-blue-50 border border-blue-200 rounded-xl p-5 mb-6">
        ...
        <div class="space-y-3">
            @foreach($decisionHistory as $decision)
                <div class="bg-white rounded-lg p-3 border border-blue-100">
                    <div class="flex justify-between items-start gap-3">
                        <p class="font-semibold text-sm">
                            Babak {{ $decision->round }}
                            @if($decision->id === $existingDecision->id)
                                <span class="text-xs bg-blue-600 text-white px-2 py-0.5 rounded-full ml-1">Terbaru</span>
                            @endif
                        </p>
                        <span class="text-xs px-2 py-1 rounded-full
                            ...
                            @else bg-red-100 text-red-800
                            @endif">
                            {{ strtoupper(str_replace('_', ' ', $decision->keputusan)) }}
                        </span>
                    </div>
                    @if($decision->catatan)
                        <p class="text-gray-600 text-sm mt-1">{{ $decision->catatan }}</p>
                    @endif
                </div>
            @endforeach
        </div>

        @if($isRevisionRound && $existingDecision->keputusan === 'approved_with_recommendation')
            <p class="text-sm text-orange-700 bg-orange-50 border border-orange-200 rounded-lg p-3 mt-3">
                <i class="fas fa-info-circle"></i>
                Pengajuan ini sedang menunggu Peneliti mengunggah revisi merespons rekomendasi di atas.
            </p>
        @endif
    </div>
@endif
...
<div class="bg-white rounded-xl shadow-sm p-5">
    <h2 class="text-xl font-semibold mb-4">
        {{ $decisionHistory->isNotEmpty() ? 'Tetapkan Keputusan Babak Baru' : 'Tetapkan Keputusan' }}
    </h2>

    @if($decisionHistory->isNotEmpty())
        <p class="text-sm text-gray-500 mb-4">
            Reviewer pada babak terbaru telah menyelesaikan telaah ulang. Silakan tetapkan keputusan
            untuk babak ini — keputusan sebelumnya tidak akan terhapus/tertimpa.
        </p>
    @endif
        ...
        <div class="mb-4">
            ...
            <div class="space-y-2">
                ...
                <label class="flex items-center gap-2 p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
                    ...
                    <span class="text-sm text-gray-500">- Proposal ditolak (keputusan final)</span>
                </label>
            </div>
...
<div class="bg-white rounded-xl shadow-sm p-5 text-center">
    @if($menungguRevisiPeneliti)
        ...
        <p class="text-sm text-gray-500 mt-1">
            ...
            dan reviewer yang relevan menyelesaikan telaah ulang.
        </p>
    @else
